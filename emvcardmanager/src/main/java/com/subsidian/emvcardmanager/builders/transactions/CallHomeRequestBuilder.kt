...
object CallHomeRequestBuilder {
    ...
    fun build (isoData: ISOData, messageFactory: MessageFactory<IsoMessage>, terminalSessionKey: String = ""): ISOMessage {
        ...
        val appVersion = "001"
        val appVersionLen: String = StringUtil.leftPadding('0', 3, appVersion.length.toString())
        val paymentChannelMode = "00000000000000000000"
        val paymentChannelModeLen: String = StringUtil.leftPadding('0', 3, paymentChannelMode.length.toString())
        val callHomeComments = "N/A" // Call – Home Merchant Information/Complaint/Comments
        val callHomeCommentsLen: String = StringUtil.leftPadding('0', 3, callHomeComments.length.toString())
        val commsServiceProvider = "WIFI/MTN/AIRTEL/GLO" // Communications Service Provider
        val commsServiceProviderLen: String = StringUtil.leftPadding('0', 3, commsServiceProvider.length.toString())
        /** Set ProcCode **/
        ...
        if (isoData.managementDataOnePrivate != null && !StringUtil.isEmpty(isoData.managementDataOnePrivate)) {
            ...
            managementDataOne
                .append("09").append(appVersionLen).append(appVersion)
                .append("10").append(paymentChannelModeLen).append(paymentChannelMode)
                .append("11").append(callHomeCommentsLen).append(callHomeComments)
                .append("12").append(commsServiceProviderLen).append(commsServiceProvider)
            message.setValue(
                62,
